...
writer = SummaryWriter('runs/res50/v102')
...
def fit(model, training_params, evaluate_params):
    ...
    for epoch in range(epochs):
        model.train()
        if evaluating_accuracy > 83:
            optimizer = torch.optim.SGD(
                model.parameters(), 
                lr=0.01, 
                momentum=0.9, 
                weight_decay=0.0001)
        if evaluating_accuracy > 90:
            optimizer = torch.optim.SGD(
                model.parameters(), 
                lr=0.001, 
                momentum=0.9, 
        ...
        with torch.no_grad():
			...
			evaluating_accuracy = 100 * evaluating_correct_num / evaluating_samples_num
            print('\taccuracy: %.3f' % (evaluating_accuracy), '%')
            writer.add_scalar('evaluating accuracy', 
            	evaluating_accuracy, 
            	epoch*len(data_loader)+i)
        torch.save(model.state_dict(), 'res50_v102.pt')
...
def main():
    ...
    print(INFO, 'preparing to load model from res50_v102.pt...')
    try:
        res50.load_state_dict(torch.load('res50_v102.pt'))
        print(INFO, 'load model successfully!')
    except:
        print(INFO, 'failed to load model, maybe there is no file named res50_v102.pt')
